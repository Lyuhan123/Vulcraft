...
public class GlStateManagerMixin {
   ...
   // glBegin/glVertex3f/glEnd capture: keep the vertices in a flat, reused
   // float[] (5 floats per vertex) instead of boxing every vertex into a
   // float[5]. The scratch ByteBuffer handed to the drawer is reused too.
   private static float[] captureData = new float[5 * 64];
   private static int captureCount = 0;
   private static ByteBuffer captureBuffer = null;
   private static float captureU;
   private static float captureV;
   // buildFormatFromPointers() rebuilds a VertexFormat (several allocations)
   // on every glDrawArrays; cache it against the pointer state it derived from.
   private static VertexFormat cachedPointerFormat;
   private static boolean cachedPointerFormatValid = false;
   private static int cachedPtrStride, cachedPtrPosOffset, cachedPtrColorOffset,
         cachedPtrUv0Offset, cachedPtrUv1Offset, cachedPtrNormalOffset;
   ...
   private static VertexFormat buildFormatFromPointers() {
      if (cachedPointerFormatValid
            && cachedPtrStride == ptrStride
            && cachedPtrPosOffset == ptrPosOffset
            && cachedPtrColorOffset == ptrColorOffset
            && cachedPtrUv0Offset == ptrUv0Offset
            && cachedPtrUv1Offset == ptrUv1Offset
            && cachedPtrNormalOffset == ptrNormalOffset) {
         return cachedPointerFormat;
      }
      ...
      cachedPointerFormat = computeFormatFromPointers();
      cachedPointerFormatValid = true;
   ...
   public static void glVertex3f(float x, float y, float z) {
      if (captureMode != 0) {
         int o = captureCount * 5;
         if (o + 5 > captureData.length) {
            captureData = Arrays.copyOf(captureData, captureData.length * 2);
         }
         captureData[o] = x;
         captureData[o + 1] = y;
         captureData[o + 2] = z;
         captureData[o + 3] = captureU;
         captureData[o + 4] = captureV;
         ++captureCount;
      }
   }
         ...
         if (captureCount >= 4) {
            if (Renderer.isRecording()) {
               ...
               if (shader != null && shader.getPipeline() != null) {
                  int vertexCount = captureCount;
                  int bytes = vertexCount * 20;

                  // Reuse the scratch buffer across glEnd calls instead of
                  // allocating/freeing native memory on every single one.
                  if (captureBuffer == null || captureBuffer.capacity() < bytes) {
                     if (captureBuffer != null) {
                        MemoryUtil.memFree(captureBuffer);
                     }
                     captureBuffer = MemoryUtil.memAlloc(bytes);
                  }

                  ByteBuffer data = captureBuffer;
                  data.clear();

                  for (int i = 0, o = 0; i < vertexCount; ++i, o += 5) {
                     data.putFloat(captureData[o]);
                     data.putFloat(captureData[o + 1]);
                     data.putFloat(captureData[o + 2]);
                     data.putFloat(captureData[o + 3]);
                     data.putFloat(captureData[o + 4]);
                  }
         ...
         if (vertexFormat != null) {
            ...
            if (shader != null) {
               ...
               if (renderer.getBoundPipeline() != null) {
                  ...
                  int byteOffset = p_187439_1_ * stride;
                  long bytesNeeded = (long) byteOffset + (long) p_187439_2_ * stride;
                  if (glBuffer.getData().limit() < bytesNeeded) {
                     return;
                  }

                  // Chunk sections (and every other VBO) are re-drawn every frame
