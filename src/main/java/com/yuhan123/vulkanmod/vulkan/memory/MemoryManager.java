...
import com.yuhan123.vulkanmod.vulkan.util.Pair;
    ...
    public void createBuffer(long size, int usage, int properties, LongBuffer pBuffer, PointerBuffer pBufferMemory) {
        try (MemoryStack stack = stackPush()) {
            ...
            if (result != VK_SUCCESS) {
                VulkanMod.LOGGER.info(String.format("Failed to create buffer with size: %.3f MB", ((float) size / BYTES_IN_MB)));
                VulkanMod.LOGGER.info(String.format("Tracked Device Memory used: %d/%d MB", getAllocatedDeviceMemoryMB(), getDeviceMemoryMB()));
                VulkanMod.LOGGER.info(getHeapStats());
...
//            imageInfo.sharingMode(VK_SHARING_MODE_CONCURRENT);
            ...
            if (result != VK_SUCCESS) {
                VulkanMod.LOGGER.info(String.format("Failed to create image with size: %dx%d", width, height));
    ...
    /**
     * Maps an allocation and returns the mapped address.
     * <p>
     * The old {@code Map()} handed back a {@code memAllocPointer} buffer that no
     * caller ever freed, leaking native memory on every buffer creation - and
     * therefore on every buffer resize too.
     */
    public long mapPointer(long allocation) {
        try (MemoryStack stack = stackPush()) {
            PointerBuffer data = stack.mallocPointer(1);
