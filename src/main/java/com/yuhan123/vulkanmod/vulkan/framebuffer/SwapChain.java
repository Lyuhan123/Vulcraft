...
import com.yuhan123.vulkanmod.render.util.MathUtil;
...
import static com.yuhan123.vulkanmod.vulkan.device.DeviceManager.vkDevice;
import static com.yuhan123.vulkanmod.vulkan.util.VUtil.UINT32_MAX;
...
public class SwapChain extends Framebuffer {
    ...
    private void createSwapChain() {
        try (MemoryStack stack = stackPush()) {
            ...
            com.yuhan123.vulkanmod.VulkanMod.LOGGER.info(
                    "Swapchain format: 0x{} (isBGRA={}) size={}x{} images={}",
                    Integer.toHexString(this.format), isBGRAformat, this.width, this.height, imageCount.get(0));

            for (int i = 0; i < pSwapchainImages.capacity(); i++) {
                long imageId = pSwapchainImages.get(i);
                long imageView = VulkanImage.createImageView(imageId, this.format, VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, isBGRAformat);
    ...
    private int getPresentMode(IntBuffer availablePresentModes) {
        ...
        VulkanMod.LOGGER.warn("Requested mode not supported: " + getDisplayModeString(requestedMode) + ": using FIFO present mode");
        return VK_PRESENT_MODE_FIFO_KHR;
    }
