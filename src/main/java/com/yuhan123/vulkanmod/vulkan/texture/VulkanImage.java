...
import static com.yuhan123.vulkanmod.vulkan.texture.SamplerManager.*;
...
public class VulkanImage {
    ...
    public static VulkanImage createTextureImage(Builder builder) {
        ...
        // Sampled-only images (textures, i.e. not colour/depth attachments) are
        // moved to SHADER_READ_ONLY by the upload batch, so the first draw that
        // samples them does not need a barrier inside the render pass.
        final int attachmentBits = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
        if ((image.usage & VK_IMAGE_USAGE_SAMPLED_BIT) != 0 && (image.usage & attachmentBits) == 0) {
            ImageUploadHelper.INSTANCE.addPendingReadOnly(image);
        }
    ...
    public static VulkanImage createWhiteTexture() {
        try (MemoryStack stack = stackPush()) {
            int i = 0xFFFFFFFF;
            // 16x16 so texelFetch(Sampler2, UV2 / 16, 0) with the lightmap fallback stays in bounds
            ByteBuffer buffer = stack.malloc(16 * 16 * 4);
            IntBuffer ints = buffer.asIntBuffer();
            for (int k = 0; k < 16 * 16; k++) ints.put(k, i);

            VulkanImage image = VulkanImage.builder(16, 16)
            ...
            return image;
//            return createTextureImage(16, 16, 4, false, false, buffer);
        }
    }
    ...
    public static long createImageView(long image, int format, int aspectFlags, int baseMipLevel, int mipLevels) {
        return createImageView(image, format, aspectFlags, baseMipLevel, mipLevels, false);
    }

    public static long createImageView(long image, int format, int aspectFlags, int baseMipLevel, int mipLevels, boolean swapRB) {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            if (swapRB) {
                // The swapchain image is B8G8R8A8 but the shaders output RGBA;
                // swap R<->B through the view so the presented colors are correct.
    ...
    public void uploadSubTextureAsync(int mipLevel, int width, int height, int xOffset, int yOffset, int unpackSkipRows, int unpackSkipPixels, int unpackRowLength, long srcPtr) {
        // GL treats UNPACK_ROW_LENGTH == 0 as "same as width"
        int rowLength = unpackRowLength != 0 ? unpackRowLength : width;
        long uploadSize = (long) (rowLength * height - unpackSkipPixels) * this.formatSize;
        ...
        // Batched upload: append the copy to the frame's upload command buffer,
        // which is submitted once in Renderer.endFrame(). Previously every
        // glTexSubImage2D got its own submit + vkWaitForFences, which drained
        // the whole GPU pipeline (lightmap, animated textures, fonts and skins
        ...
        // any render pass. Until then the draw path must not transition it.
        ImageUploadHelper.INSTANCE.addPendingReadOnly(this);
    }
    ...
    public void readOnlyLayout() {
        ...
        // An open upload batch will transition this image outside the render
        // pass. Transitioning here too would emit a barrier inside an active
        // subpass, which is illegal without a self-dependency.
        if (ImageUploadHelper.INSTANCE.isPendingReadOnly(this))
            return;
...
//            System.out.println("new layout is equal to current layout");
    ...
    public void doFree() {
        ...
        // Destroy each per-level view; the old code passed mainImageView here,
        // so every level view leaked while the main view was destroyed N+1 times.
        if (this.levelImageViews != null) {
            for (long imageView : this.levelImageViews) {
