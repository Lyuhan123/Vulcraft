...
public class VkGlBuffer {
    ...
    /**
     * Persistent host-visible mirror of `data`, bound directly by
     * glDrawArrays instead of re-copying the vertex range into the per-frame
     * vertex buffer on every draw.
     *
     * 1.12.2 draws chunk sections straight from VBOs with glDrawArrays, and the
     * VBO contents only change when the section is rebuilt. Copying every
     * section into the per-frame buffer each frame meant tens of megabytes of
     * memcpy per frame; binding the upload once removes that entirely.
     *
     * Re-uploads allocate a fresh buffer and defer the old one: frames already
     * in flight keep reading the old buffer, and MemoryManager only destroys it
     * after the fence of a later frame has been waited on (queue submission
     * order guarantees every earlier frame - and thus every reader - finished).
     */
    ...
    public VertexBuffer getPersistentVertexBuffer() {
        return this.vertexBuffer;
    }

    /** Mirrors `data` into a persistent vertex buffer. `data` must be rewound. */
    private void uploadPersistent() {
        if (this.data == null) {
            return;
        }

        final int size = this.data.limit();
        if (size <= 0) {
            return;
        }
        ...
        try {
            ...
            // Out of device/host memory: fall back to the per-frame copy path.
