...
//            return createTextureImage(16, 16, 4, false, false, buffer);
    ...
    public void uploadSubTextureAsync(int mipLevel, int width, int height, int xOffset, int yOffset, int unpackSkipRows, int unpackSkipPixels, int unpackRowLength, long srcPtr) {
        ...
        // Non-batched upload: record the copy into a fresh command buffer and
        // submit + wait immediately, so the staging data is consumed right away
        // and the texture is ready for the next frame.
        CommandPool.CommandBuffer cmdBuffer = DeviceManager.getGraphicsQueue().beginCommands();
        try (MemoryStack stack = stackPush()) {
        ...
        long fence = DeviceManager.getGraphicsQueue().submitCommands(cmdBuffer);
        Synchronization.INSTANCE.addCommandBuffer(cmdBuffer);
